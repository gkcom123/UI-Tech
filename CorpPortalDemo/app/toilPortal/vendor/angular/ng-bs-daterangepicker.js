...
angular.module('ngBootstrap', []).directive('input', function ($compile, $parse, $rootScope) {
	return {
		restrict: 'E',
		require: '?ngModel',
		link: function ($scope, $element, $attributes, ngModel) {
			...
			$($element).daterangepicker(options, function(start, end) {
				//console.log(" outside appllyFunciton");
				$scope.$apply(function () {
					//console.log("got appllyCall");

					ngModel.$setViewValue({ startDate: start, endDate: end });
					ngModel.$render();					
					$rootScope.$broadcast('DateRangePickerValueChangedMessage',{});
				});
			});			
		}
	};
